...
class CraftingEngine:
    ...
    def get_ui_state(
        ...
        # 与 GDScript 端 SEMANTICS.md 对齐:None 也用 "" 表示(UI 端 "falsy" 即可,
        # 但显式 "" 便于序列化 / 模板替换)
        if check.blocked is None:
            blocked_zh: Optional[str] = ""
        else:
            blocked_zh = _BLOCK_REASON_ZH.get(check.blocked, check.blocked)
        ...
        return {
            ...
            "blocked_reason": blocked_zh,  # 中文物文("" 表示通过)
