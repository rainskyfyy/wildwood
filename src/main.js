...
import { loadImage, preloadImages, isReady, getOrFallback } from './render/image-loader.js';
...
// M2.14: monster system
import { MonsterManager } from './monster/monster-manager.js';
import { drawMonster } from './render/tile-renderer.js';
// monstersData is filled in asynchronously by fetch() in bootGame()
// — mirrors the image-loader pattern (kick off the load, render the
// demo immediately with a procedural fallback, swap to the real data
// once it arrives).
...
export function bootGame(canvas) {
  ...
  // 4b. M2.14: monster manager. Lazy-loads per-state PNGs through the
  ...
  //     arrives, the manager is empty (so the demo runs as a 0-monster
  //     M5 build).
  ...
  if (!monstersData) {
    fetch('./data/monsters.json').then(r => r.ok ? r.json() : null)
      .then(data => {
        if (!data) return;
        monstersData = data;
        // Replace the manager's view of the data and spawn now.
  ...
  function frame(now) {
    ...
    hud.update();
    updateBuildingInput();
    // M2.14: monster AI tick (think + move + axis-separate).
    ...
    // Render.
    if (imagesReady) {
      render(ctx, canvas, world, decor, transitions, player, camera, buildingMgr, monsterMgr);
...
function render(ctx, canvas, world, decor, transitions, player, camera, mgr, monsterMgr) {
  ...
  // Pass 2: depth-sorted list of decor + buildings + monsters + player.
  const drawables = [];
  ...
  // M2.14: monsters interleave between buildings and the player.
  if (monsterMgr) {
  ...
  for (const it of drawables) {
    if (it.kind === 'decor') {
      ...
      const sx = s.x + offsetX;
      const sy = s.y + offsetY;
      const sprite = monsterMgr.resolveSprite(m);
      drawMonster(ctx, sx, sy, m, sprite);
    } else {
      const s = worldToScreen(it.ref.x, it.ref.y);
      const sx = s.x + offsetX;
      const sy = s.y + offsetY;
