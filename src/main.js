 ...
 * v1.0.3 — M2.10d: depletion + transform
 * v1.0.4 — M2.10e: three-stage growth (time-driven) for tree / dead_tree /
 *                   berry_bush. Spawned as stage 0 (sprout), grow into
 *                   stage 1 (mature), then into terminal stage 2 (old).
 *                   Stage 0 drops are minor; stage 2 includes a rare seed.
 *                   Harvest resets the cycle to stage 0 and waits the
 *                   harvested stage's regrowTime before re-pick.
 */
'use strict';
...
export function bootGame(canvas) {
  ...
  const regrow = new RegrowManager({
    entities: resources,
    onRegrow: (e) => {
      // v1.0.4: distinguish growth-stage respawn (cycle restart) from a
      // simple regrow. Growth-capable entities go back to stage 0 (sprout)
      // after harvest; non-growth entities just respawn at the same id.
      if (e.isGrowthCapable) {
        lastRegrowBanner = `${e.def.name} 周期完成 → 重生为 ${e._rootId} 幼苗`;
      } else {
  ...
  let lastStageBanner = null;
  let lastStageUntil = 0;
  ...
  function frame(now) {
    ...
    // v1.0.4: capture per-entity stage index BEFORE regrow.update() so we
    // can detect a growth-stage advance and emit a banner (e.g. sapling→tree).
    for (const e of resources) e._lastKnownStage = e.currentStageIndex;
    regrow.update(now);
    for (const e of resources) {
      if (e.isGrowthCapable && e._lastKnownStage != null
          && e.currentStageIndex !== e._lastKnownStage) {
        lastStageBanner = `${e.def.name} 进阶:第 ${e._lastKnownStage} → ${e.currentStageIndex} 阶段`;
        lastStageUntil = performance.now() + 1800;
      }
    }
    ...
    if (lastStageBanner && now < lastStageUntil) {
      // v1.0.4: growth-stage advance banner (e.g. sapling→mature tree).
      // Gold to signal the player that richer drops are now available.
      ctx.fillStyle = 'rgba(0,0,0,0.7)';
      ctx.fillRect(canvas.width/2 - 130, 186, 260, 24);
      ctx.fillStyle = '#f4c84a';
      ctx.font = 'bold 12px sans-serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(`★ ${lastStageBanner}`, canvas.width/2, 198);
    } else {
      lastStageBanner = null;
    }
