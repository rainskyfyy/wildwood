...
Files changed in v0.8.0a:
  ...
  - demo-v080a.html                 (NEW — interactive before/after demo)

Pushes to a feature branch `feat/v0.8.0a-pass-through-freeze`. Idempotent:
first run creates the branch on top of current main HEAD; subsequent runs
become incremental fix-up commits on top of the existing branch HEAD.
"""
...
def main():
    ...
    # 1. main HEAD(用于判断分支是否存在 / base 选什么)
    main_sha = get_ref('heads/main')
    if not main_sha:
        print('ERROR: main ref not found', file=sys.stderr)
        sys.exit(1)
    print(f'  main HEAD:   {main_sha[:7]}')

    # 2. base:已存在 branch → 增量;否则 → main HEAD 起步
    ...
    if branch_sha:
        ...
        print(f'  branch {BRANCH} not found → starting from main HEAD')
    ...
    if is_incremental:
        msg = (
            'v0.8.0a (demo): 补 demo-v080a.html 浏览器内 before/after 对比\n\n'
            '纯静态 HTML + 内嵌 JS,演示:\n'
            '  - BEFORE:game 字段无任何保护,任何 mutation 静默通过\n'
            '  - AFTER:装配层 freeze 后,换引用 / delete / redefine 抛 TypeError\n'
            '  - 实例方法(inventory.add)与只读访问仍正常工作\n\n'
            '根 commit: ' + base_sha
        )
    else:
        msg = (
            ...
            '  - demo-v080a.html (新):浏览器内 before/after 对比 demo。\n\n'
            '验收:\n'
    ...
    # 7. push: POST 新 ref 或 PATCH 已有 ref
    if is_incremental:
        r = requests.patch(
