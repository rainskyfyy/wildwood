...
v0.8.0d · metrics hygiene 升级: 走 metrics_helpers,加 fail-fast startup check, 走 metrics/update_roadmap.jsonl
...
# === v0.8.0d: 接入 tools/ 下的 metrics_helpers 和 startup_check ===
# 把 tools/ 加入 sys.path,以便 import metrics_helpers / startup_check
...
try:
    from metrics_helpers import (
        write_metric as _helpers_write_metric,
        read_recent as _helpers_read_recent,
        SCHEMA_VERSION,
    )
    from startup_check import check_credentials_warn_only, DEFAULT_BOSS_OPEN_ID
    _HAS_TOOLS = True
except ImportError as e:
    _HAS_TOOLS = False
    print(f"  WARN: tools/ 未找到 ({e}),降级到旧 SYNC_METRICS_PATH", file=sys.stderr)
...
# v0.8.0d: 新路径是 ~/.aily/workspace/wildwood/metrics/update_roadmap.jsonl (走 metrics_helpers)
# 保留 SYNC_METRICS_PATH 作为 legacy fallback,tools/ 不可用时降级
SYNC_METRICS_PATH   = WORKSPACE_DIR / "sync_metrics.jsonl"  # LEGACY 兜底,tools/ 不可用时使用
...
# -------------------- 可观测性层 (v0.8.0d metrics hygiene 升级) --------------------
SCRIPT_SOURCE = "update_roadmap.py"  # 写入 metrics_helpers 后会被 helper 强制覆盖

# 已知非本脚本的污染源,过滤掉避免被计入健康指标(v0.8 仍保留,作为 legacy 兜底)
...
def record_sync_metric(start_ts, status, duration_seconds, commit_count, task_count,
                       commit_sha="", error=""):
    """v0.8.0d: 走 metrics_helpers.write_metric,append-only + schema_version + source。

    若 tools/ 未就位,降级到旧 SYNC_METRICS_PATH 写共享文件(向后兼容)。
    """
    if _HAS_TOOLS:
        return _helpers_write_metric(
...
def read_recent_metrics(n=10, source_filter=None):
    """
    v0.8.0d: 走 metrics_helpers.read_recent(优先读 metrics/update_roadmap.jsonl,
    合并读 legacy sync_metrics.jsonl 兼容历史),自动按 source 过滤并排除 fixture。

    兼容 source_filter 参数(legacy 命名):实际效果固定为本脚本 source,不再做白名单。
    """
    if _HAS_TOOLS:
        return _helpers_read_recent(source=SCRIPT_SOURCE, n=n, include_legacy=True)
    # 降级路径(legacy): 直接读 sync_metrics.jsonl
...
# -------------------- main --------------------
def main():
    parser = argparse.ArgumentParser(description="Wildwood 路线图同步器 v0.8.0d")
    ...
    parser.add_argument("--strict", action="store_true", help="v0.8 严格模式: 缺凭据立即退出 2 (v0.7 行为: 静默降级)")
    args = parser.parse_args()

    parent_task_id = args.parent_task
    output_path = args.output

    # === v0.8.0d: 启动期 fail-fast 校验 ===
    will_push = bool(args.push or (GH_TOKEN and not SYNC_DRY_RUN))
    will_alert = True  # update_roadmap 总是会触发告警逻辑(连续失败时)
    if args.strict or _HAS_TOOLS:
        # 默认开启 fail-fast(v0.8 起强制),避免 v0.7.3a 那种 7 次空转
        ...
        # 兜底:tools/ 不可用,只 warn 不退出
        ok, status_d = check_credentials_warn_only(push=will_push, alert=will_alert, script_name=__file__)
        if not ok:
            print(f"  WARN: 凭据校验未通过,但 tools/ 不可用,继续执行(可能后续失败)")

    print(f"=== Wildwood Roadmap Sync v0.8.0d ===")
