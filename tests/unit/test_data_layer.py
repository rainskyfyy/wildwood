...
class TestJsonFileStore(DataStoreContractMixin, unittest.TestCase):
    ...
    def test_files_created(self):
        ...
        # M2.6 分块:1 profile + 1 inventory = 2 文件
        profile_files = list((save_dir / "profiles").iterdir())
        self.assertEqual(len(profile_files), 2)
        names = {p.name for p in profile_files}
        # profile 主文件 + inventory chunk 文件
        self.assertEqual(len([n for n in names if n.endswith("_inventory.json")]), 1)
        self.assertEqual(len([n for n in names if not n.endswith("_inventory.json")]), 1)
