...
- **34 配方**(9 tool / 7 equip / 11 food / 7 building;8 NONE / 15 WORKBENCH / 11 COOKPOT)超过 30 配方硬要求
- **数据层** `core/abstract/crafting/schemas.py`:Ingredient / Recipe(frozen,id 强校验)/ StationType 枚举 / RecipeCategory 枚举 / CraftingResult
- **配方书** `core/abstract/crafting/recipe_book.py`:RecipeBook.default_book() + with_recipe / all / find_by_id / by_category / by_station
- **抽象接口** `inventory_view.py` + `station_probe.py`:InventoryView Protocol + DictInventoryView + FailingInventoryView;StationProbe Protocol + StaticStationProbe
- **合成引擎** `crafting_engine.py`:check_can_craft(纯查询)/ craft(扣材料+产出,补偿回滚)/ get_ui_state(中文物文/缺料/工作站门槛)
- **GDScript 薄包装** `crafting.gd` + SEMANTICS.md:class_name WildwoodCrafting,3 主 API + apply_craft_result 辅助,Python↔GDScript 9 节对齐契约
- **GDScript 配方数据** `recipe_book_data.gd`:AUTO-GENERATED,单一真相源是 Python RecipeBook
- **Demo 场景** `scenes/crafting_demo.tscn` + `scripts/crafting_demo.gd`:动态 UI 演示 4 项验收
- **性能基准**:`run_m29_perf.py` 单次 craft p99=0.009ms(400ms 预算,44000× 余量);34 配方全表 re-check p99=0.159ms
- **测试 84 个全过**:test_crafting 11 + test_recipe_book 16 + test_crafting_abstract 21 + test_crafting_engine 27 + test_gd_crafting_parity 9 + test_m29_demo_e2e 9
- **验收脚本** `run_m29_tests.sh`:一键跑 7 步,CI 友好

### 修复

- **跨端契约一致性 bug**:`get_ui_state.blocked_reason` None → `""`,对齐 GDScript SEMANTICS 契约
- 同步更新 `test_crafting_engine.py` 2 处断言

## [0.7.0] - 2026-08-20
  - `Ingredient`(frozen dataclass):`item_id` / `count`(> 0 校验)
  - `Recipe`(frozen):`id` 强校验 `^craft\.(tool|equipment|food|building)\.[a-z][a-z0-9_]*$`
  - `StationType` 枚举:`NONE` / `WORKBENCH` / `COOKPOT`
  - `RecipeCategory` 枚举:`TOOL` / `EQUIPMENT` / `FOOD` / `BUILDING`
  - `CraftingResult`(frozen):`recipe_id` / `produced` / `consumed`
  - `CURRENT_RECIPE_SCHEMA_VERSION = "1.0.0"`,`parse_version()` + `is_compatible()`(从 M1.4 移植)
- **配方书**(`core/abstract/crafting/recipe_book.py`)
  - **34 配方**(超过 30 最低要求):9 tool / 7 equipment / 11 food / 7 building
  - 分布:8 NONE(自由合成) / 15 WORKBENCH / 11 COOKPOT
  - `RecipeBook.default_book()` + `with_recipe()` / `all()` / `find_by_id()` / `by_category()` / `by_station()` / `known_result_item_ids()` / `known_ingredient_item_ids()`
  - `DuplicateRecipeError`,模块加载时断言 ≥30 + id 唯一 + id 前缀对齐 category
- **抽象接口**(`core/abstract/crafting/inventory_view.py` + `station_probe.py`)
  - `InventoryView` Protocol(runtime_checkable):`get_count` / `consume` / `add`
  - `DictInventoryView` 参考实现 + `FailingInventoryView` 测试桩
  - `StationProbe` Protocol:`has_station`
  - `StaticStationProbe` 不可变:`with_station()` / `without_station()` 返新实例
  - M2.2 / M2.3 接入点:实现 InventoryView / StationProbe 即可对接
- **合成引擎**(`core/abstract/crafting/crafting_engine.py`)
  - `CraftingEngine.check_can_craft()`:纯查询,不动库存
  - `CraftingEngine.craft()`:扣材料 + 加产出,带补偿回滚(顺序 consume + 失败 add-back)
  - `CraftingEngine.get_ui_state()`:返 UI 状态 dict(中文物文 / 缺料详情 / 工作站门槛)
  - `CraftingError` 异常 + `CheckResult`(frozen)检查结果
  - `_ITEM_LABELS` 36 项中文映射 + `_BLOCK_REASON_ZH` 工作站门槛文案
- **GDScript 薄包装**(`core/abstract/crafting/crafting.gd` + `SEMANTICS.md`)
  - `class_name WildwoodCrafting`,纯 static func 集合
  - 3 主 API:`check_can_craft` / `craft` / `get_ui_state`(语义对齐 Python)
  - 额外 `apply_craft_result()` 辅助事务化(扣 + 加原子)
  - 36 项 `ITEM_LABELS` + `BLOCK_REASON_ZH` 中文物文
  - **SEMANTICS.md**(199 行):Python ↔ GDScript 9 节对齐契约,任何字段/语义变更先改本文档
- **GDScript 配方数据**(`core/abstract/crafting/recipe_book_data.gd` + `_dump_recipes.py`)
  - AUTO-GENERATED by `_dump_recipes.py`,**单一真相源**:Python 端 `RecipeBook` 改后重跑 dump
  - 34 配方 const `RECIPES` + 静态方法 `default_book` / `find_by_id` / `by_station` / `by_category`
- **Demo 场景**(`scenes/crafting_demo.tscn` + `scripts/crafting_demo.gd`)
  - 动态构建 UI:header(标题 + 切换工作台/烹饪锅/+5 材料/清空) + 库存 + 工作站 + 34 配方滚动列表 + footer(toast + perf)
  - 演示所有 4 项验收:配方全显示 / 按钮可点 + 灰显 / 合成即时反馈 / 切换工作站实时变状态
  - Godot 编辑器打开 .tscn 作主场景 F5 运行
- **性能基准**(`tests/scripts/run_m29_perf.py`)
  - 单次 craft p99 = **0.009ms**(预算 400ms → 44,000× 余量)
  - 34 配方全表 re-check p99 = **0.159ms**(单项 1.29µs)
  - 大库存(50 item)craft p99 = **0.007ms**
  - 内部目标 p99 < 50ms(8× 余量)
- **测试**(共 84 个测试,全部 PASS)
  - `test_crafting.py`(11):schema / dataclass 基础
  - `test_recipe_book.py`(16):34 配方注册 + 查询 + 唯一性
  - `test_crafting_abstract.py`(21):InventoryView / StationProbe 接口合约
  - `test_crafting_engine.py`(27):check / craft / get_ui_state + 性能
  - `test_gd_crafting_parity.py`(9):Python ↔ GDScript 跨端对齐
  - `test_m29_demo_e2e.py`(9):4 项验收端到端模拟
- **验收脚本**(`tests/scripts/run_m29_tests.sh`):一键跑全部 7 步,CI 友好

### 修复

- **跨端契约一致性 bug**(`core/abstract/crafting/crafting_engine.py`)
  - `get_ui_state` 的 `blocked_reason` 字段:None → `""`(空字符串)
  - 与 GDScript 端 `SEMANTICS.md` 第 4 节契约对齐
  - 修复理由:Python 端旧实现 `check.blocked` 为 None 时返 None,导致 GDScript 端 demo `if blocked_reason == ""` 判断失败
  - 同步更新 `test_crafting_engine.py` 2 处断言(`assertIsNone` → `assertEqual(_, "")`)

### 关键路径意义

- 解锁 M2.7(合成栏 UI,部分依赖) + M2.13(UI 设计师接入)
- 不延期 M2.1 / M2.2 / M2.3 / M2.6:这些任务不依赖 M2.9,但 M2.9 提供 inventory / station 抽象接口作为它们的对接点
- M3.7-3.8 联机健壮性不直接依赖 M2.9(联机层是网络,与合成逻辑正交)
